...
import { Product, Sort, Collections } from "@/components";
...
const Accessories = ({ products }) => {
  ...
  useEffect(() => {
    const regex = new RegExp("accessories", "i"); // 'i' flag for case-insensitive search
    const filteredStuff = products.filter((prod) =>
      regex.test(prod.collection)
    );
    setFilteredResults(filteredStuff);
    setSortType("arr");
  }, []);

  const newArray = filteredResults;
